...
for uni = 1:length(siteNames)
    ...
    for s = 1:size(subj,2)
        ...
        load(subj_file{s});
        output_file_sCz = strcat('../mi3_Preprocessed_sCz/mi3_',subj{s},'_sCz.mat');
        ...
        sCz = preprocess_modified_splitHalf(data,'/mi3/','no');
        
        %% Save preprocessed data
        % Save analyzed data for Cz:
        disp('   Saving analyzed data to a MAT file...');
        save(output_file_sCz,'sCz');
        disp('      Analyzed data saved as:  ');
        disp(['         ', output_file_sCz]);
                
        %% Clear all variables except a few that we need
        clearvars -except uni s N Fc1 Fc2 cutOff baseline_start_in_ms start_in_ms stop_in_ms folderName original_path siteNames subj_file subj specificSubjs daFolder folder_mi3_Analyses 
    end
    
    cd ../../../
end
clearvars -except folderName original_path siteNames  daFolder folder_mi3_Analyses 
