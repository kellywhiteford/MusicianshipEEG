...
function [sCz] = preprocess_modified_splitHalf(DATA,task,makePlot)
% Input
...
%% Single-electrode analysis - Split half
...
% Does this for each half of data.

nTrials = Cz.UsableEvents;

h1_inds = 1:round(nTrials/2);
h2_inds = round(nTrials/2)+1:nTrials;
...
h1_t1_ind = t1_ind(t1_ind <= round(nTrials/2)); % indices of trigger 1 events for FIRST HALF of data
h1_t2_ind = t2_ind(t2_ind <= round(nTrials/2)); % indices of trigger 2 events for FIRST HALF of data

h2_t1_ind = t1_ind(t1_ind > round(nTrials/2)); % indices of trigger 1 events for SECOND HALF of data
h2_t2_ind = t2_ind(t2_ind > round(nTrials/2)); % indices of trigger 2 events for SECOND HALF of data
...
h1_avgT1 = mean(Cz.Epoched(:,:,h1_t1_ind),3); % FIRST HALF Trigger 1 averaged across events
h1_avgT2 = mean(Cz.Epoched(:,:,h1_t2_ind),3); % FIRST HALF Trigger 2 averaged across events

h2_avgT1 = mean(Cz.Epoched(:,:,h2_t1_ind),3); % SECOND HALF Trigger 1 averaged across events
h2_avgT2 = mean(Cz.Epoched(:,:,h2_t2_ind),3); % SECOND HALF Trigger 2 averaged across events
...
h1_b_avgT1 = mean(Cz.Baseline(:,:,h1_t1_ind),3); % FIRST HALF Trigger 1: Average across events
h1_b_avgT2 = mean(Cz.Baseline(:,:,h1_t2_ind),3); % FIRST HALF Trigger 2: Average across events

h2_b_avgT1 = mean(Cz.Baseline(:,:,h2_t1_ind),3); % SECOND HALF Trigger 1: Average across events
h2_b_avgT2 = mean(Cz.Baseline(:,:,h2_t2_ind),3); % SECOND HALF Trigger 2: Average across events


sCz.SumAvg = avgT1 + avgT2; % Summed response across the two polarities for Cz
sCz.SumAvg_Baseline = b_avgT1 + b_avgT2; % Summed response across the two polarities for baseline Cz

% FIRST HALF
sCz.SumAvg_h1 = h1_avgT1 + h1_avgT2;
sCz.Sumavg_Baseline_h1 = h1_b_avgT1 + h1_b_avgT2;

% SECOND HALF
sCz.SumAvg_h2 = h2_avgT1 + h2_avgT2;
sCz.Sumavg_Baseline_h2 = h2_b_avgT1 + h2_b_avgT2;


sCz.UsableEvents = nTrials;
sCz.Fs = Cz.Fs;
sCz.FixedDelay_ms = Cz.FixedDelay_ms;
