function [f0Max, f0amp_avg, allFFT_mag] = slidingFFT_mi3_s(dat,baseline,Fs,delay, isStim, shouldPlot)
% Performs the sliding window FFT analysis described in Wong et al. (2007,
% Supplementary Methods). 
% NOTE: Only use this version for split-half reliability analyses. Does not
% include the spectral noise floor criterion.
...
%              is the EEG response.
% shouldPlot : String indicating 'yes' if individual data should be plotted
...
for b = 1:numBins
    ...
    if b==1
        noise_pos(too_high) = []; % Removes magnitudes of freqs in noise floor that are far away from the F0
        noise_pos(too_low) = []; % Removes magnitudes of freqs in noise floor that are far away from the F0
    end
    ...
    if isempty(y)
        warning(['Bin ' num2str(b) ' has no frequencies with magnitudes above the noise floor.'])
        max(noise_pos)
        min(noise_pos)
    end
