...
elseif isnumeric(data) % numeric input   
    ...
    if hascategories && numel(data) == numel(cats)    
        ...
        catnames = (unique(cats,'stable')); % this ignores categories without any data - EDITED BY KLW SO THAT CATNAMES ORDER IS THE SAME AS CATS
        catnames_labels = {};
        for n = 1:length(catnames)
            thisCat = catnames(n);
        ...
        set(gca,'box','off'); % klw changed 'on' to 'off'
        return
    else
        data = {data};
    end
